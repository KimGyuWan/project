...
export default function PostsPage() {
  ...
  const fetchPosts = useCallback(async (cursor: string | null = null, reset: boolean = false, signal?: AbortSignal) => {
    // 요청이 취소되었는지 확인
    if (signal?.aborted) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || error.message || 'Failed to fetch posts');
      }
      ...
      if (reset) {
        ...
        setTotal(Array.isArray(postsData) ? postsData.length : 0);
        setHasLoadedOnce(true);
      } else {
        setPosts(prev => [...prev, ...(Array.isArray(postsData) ? postsData : [])]);
        setTotal(prev => prev + (Array.isArray(postsData) ? postsData.length : 0));
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // fetchPosts 함수를 직접 사용
    fetchPosts(null, true, abortController.signal);
  ...
  }, [search, category, sortField, sortOrder, token, authLoading, fetchPosts]);
